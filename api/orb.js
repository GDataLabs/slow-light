...
VISUALIZATION EXERCISE: Gather information through gentle sensory imagination. Invite noticing a sensation, texture, distance, movement, or support, one at a time. Never ask the person to design a scene, choose camera settings, or instruct a video generator. Keep their eyes open if preferred; if imagery is difficult, ordinary words or sensations are enough. Reflect images tentatively; do not equate an image with a diagnosis or assume it means relief.
...
- ask_feelings: invite them to imagine their current feeling as a texture or a movement; ask what they notice. Ordinary feeling words are welcome if imagining is difficult.
...
- ask_destination: invite them to imagine a moment with a little more ease, and ask what quality they would like to feel there.
- scene_intro: when visualEnabled is true, invent a specific natural place from the visitor's sensory images, initialWords, rawAnswer and desired feeling. There is no preset scene catalogue or feeling-to-location mapping. The spoken introduction and visual must describe the SAME newly imagined place. When visualEnabled is false, evoke the app's simple fallback location in person.sceneKey.
...
- scene_update: reflect what they noticed in the visualization exercise, without interpreting it as a diagnosis. Their words guide the imagery in the background; do not mention clips, prompts or rendering in the spoken line. Never claim a change has already appeared.
...
module.exports = async (req, res) => {
  ...
  const openingImagery = body.visualEnabled === true && stage === "scene_intro"
    ? findImagery(clip(person.placeWords, 120), clip(person.initialWords, 300), clip(person.rawAnswer, 300)) : null;
  const chosenPlace = body.visualEnabled === true ? clip(person.placeWords, 120) : "";
  const ctx = {
    stage,
    ...
    // the place they already named in their first answer leads the opening scene
    openingImagery: openingImagery ? openingImagery.phrase : null,
    openingVariation: body.visualEnabled === true && stage === "scene_intro" && !openingImagery && !chosenPlace ? {
      scale: ["an intimate nearby natural detail", "a sheltered middle-distance view", "an expansive open view"][randomInt(3)],
      light: ["soft reflected daylight", "clear high-altitude light", "gentle slanting light", "soft luminous overcast light"][randomInt(4)],
      focus: ["layered natural textures", "a distinctive geological form", "moving water or air", "the meeting of two natural materials"][randomInt(4)]
    } : null,
    ...
    person: {
      ...
      placeWords: chosenPlace,
      recentPlaces: body.visualEnabled === true && stage === "scene_intro" ? [].concat(person.recentPlaces || []).slice(-5).map(x => clip(x, 90)) : [],
      ...
      sceneDescription: body.visualEnabled === true ? clip(person.sceneDescription,900) : "",
  ...
  try {
    ...
    const visual = body.visualEnabled === true && !out.crisis &&
      ...
      typeof out.visual === "string" && out.visual.trim() && process.env.FAL_KEY && (out.visual = anchorVisual(out.visual.trim(), openingImagery))
    ...
    res.status(200).json({
      ...
      soundscape: visual && openingImagery ? openingImagery.soundscape : visual && ["underwater","sky","forest","shore","rain","meadow","dawn","quiet"].includes(out.soundscape) ? out.soundscape : null,
