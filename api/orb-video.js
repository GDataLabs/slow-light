  ...
  try {
    if (body.action === 'submit') {
      ...
      const j = await call(queue + 'minimax/h3-max/' + (personalized ? 'reference-to-video' : continuity ? 'image-to-video' : 'text-to-video'), 'POST', {
        ...
        prompt: direction + (personalized ? " Image 1 is the visitor’s approved appearance reference. Show that same person with consistent facial features, hairstyle and clothing; do not infer their mood or health from the photo. " + (continuity ? "Image 2 is the final frame of the preceding clip. Begin from its composition and preserve both the person and environment while continuing the requested action." : "Place the person naturally within the requested scene, seen from an external viewpoint.") : "") + " Audio: quiet natural environmental sounds synchronized with what is visible. Gentle restrained volume, no sudden loud sounds. No speech, dialogue, narration, singing, or music. Do not carry water sounds into an aerial or dry environment.", duration, resolution: ['480P', '768P', '1080P'].includes(body.resolution) ? body.resolution : '480P',
